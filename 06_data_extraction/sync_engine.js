// Complete synchronization engine for Covidence API v1
window.__syncAllStudies = async function(startIdx = 0, endIdx = 63) {
  const payloads = window.__COVIDENCE_PAYLOADS;
  const sids = window.__COVIDENCE_SIDS.slice(startIdx, endIdx);
  ...
  for (let i = 0; i < sids.length; i++) {
    ...
    try {
      ...
      // 1. Save Baseline Characteristics
      ...
      // B. Pain intensity at rest (Outcome 2)
      ...
      // C. Intraoperative Opioid (Outcome 3)
      ...
      resultsLog.push({ index: startIdx + i + 1, sid, label: p.label, bcSaved, resSaved, status: 'ok' });
    } catch (err) {
      resultsLog.push({ index: startIdx + i + 1, sid, status: 'error', error: err.message });
    }
  }
