# Unresolved decisions requiring reviewer action (v37, 20 September 2026)
...
|---|---|---|---|
| A1 | **Evidence-base completeness.** 122 full texts were excluded as "wrong outcomes" (108 on the registration date), although registered eligibility admits any eligible outcome. Triage flags 84 for priority re-screening. | USER DEFERRED reviewer screening package; current 70-report base used as-is; GRADE notes a review-level selection problem, not publication bias | Dual independent re-screen of `02_DECISIONS/exclusion_rescreen_triage.csv` (84 priority + 29 secondary) against the 20 Aug 2026 criteria; extract any new trials; rerun the full chain |
...
| A3 | **Registration identifier.** Supplied PDF shows no identifier of its own; CRD420251090635 is a similar broader review | CLOSED 20 Sep: official record CRD420261452908 matches exact title, authors, dates and version. Dashboard remains unchanged/stale. | Evidence: `02_DECISIONS/v37_external_verification.md`; chronology disclosure still required |
...
| A5 | **RoB 2 coverage and sign-off.** 97 of 761 results have exact assessments; many members of non-sensitivity bodies are UNLINKED (e.g., all 4 current vomiting24_TEAS_sham contrasts, all bowel-sound/defecation TEAS contrasts, Szmit usual-care opioid). Linker placeholders do not capture historical adoption records; version-specific conflicts remain. | Unlinked shown as UNLINKED, never imputed | Result-specific RoB 2 for unlinked members of non-sensitivity bodies; record assessor/date for existing ones |
| A6 | **Dashboard.** `dashboard/` and `_site/` still present the superseded k=4 TEAS/sham (−13.99), the k=7 combined TEAS+EA (−9.91) and the old registration ID | Not rebuilt; banner removed at user request 17 Sep (merge `9b70766`); do not re-add | Rebuild from `06_REPORTS/model_analysis_manifest.csv` only after A1–A5; semantic claim audit |
