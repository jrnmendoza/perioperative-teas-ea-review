...
def t_generated_artifacts_are_gated_on_their_generators():
    """
    ADDED 2026-09-12. A cheap structural check, NOT the drift check itself.

    The drift check regenerates bundles and restores them, so it must not run
    inside this validator -- two concurrent runs would fight over the same files.
    It lives in scripts/check_generated_artifacts.py and runs in CI before the
    Pages artifact is uploaded. What this asserts is that the gate still exists,
    still covers every bundle that has a single-purpose generator, and is still
    wired into the deploy workflow.

    The gate was added because three bundles had drifted at once:
      primary_pathway.js   three admitted trials recorded as contributing to nothing
      limitations.js       3 unconvertible contrasts when the audit held 4
      prisma_checklist.js  2 recorded amendments when there were 4, and an
                           understated computed-not-reported count
    plus v34_data.js, which was 24 RoB 2 assessments behind because its generator
    had been aborting on a stale hash lock since commit eeca477.
    """
    probs = []
    checker = ROOT / "scripts" / "check_generated_artifacts.py"
    if not checker.exists():
        check("t_generated_artifacts_are_gated_on_their_generators", False,
              "scripts/check_generated_artifacts.py is gone; nothing stops a generated "
              "bundle drifting from its generator")
        return
    src = checker.read_text(encoding="utf-8")

    wf = ROOT / ".github" / "workflows" / "deploy-pages.yml"
    if not wf.exists() or "check_generated_artifacts.py" not in wf.read_text(encoding="utf-8"):
        probs.append("the deploy workflow does not run check_generated_artifacts.py, so drift "
                     "would reach the published site")

    # Every bundle whose header names a generator must be covered by the gate.
    declared = {}
    for f in sorted((ROOT / "dashboard").glob("*.js")):
        head = f.read_text(encoding="utf-8", errors="replace")[:1200]
        m = re.search(r"(?:GENERATED|Generated) by\s+(scripts/[\w/.]+\.py)", head) or \
            re.search(r'"generated_by":\s*"(scripts/[\w/.]+\.py)"', head)
        if m:
            declared[f.name] = m.group(1)
    for bundle, gen in sorted(declared.items()):
        if f'"{bundle}"' not in src:
            probs.append(f"{bundle} says it is generated by {gen} but the drift gate does not "
                         f"cover it")
    # And the gate must not claim to cover something that no longer exists.
    for m in re.finditer(r'\("(scripts/[\w/.]+\.py)",', src):
        if not (ROOT / m.group(1)).exists():
            probs.append(f"the drift gate lists {m.group(1)}, which does not exist")

    check("t_generated_artifacts_are_gated_on_their_generators", not probs, "\n".join(probs))
...
def main() -> int:
    ...
    sections = [
        ...
        ("Final reconciliation pass", [t_ea_comparator_not_mislabelled_sham, t_combined_not_labelled_primary,
                                       ...
                                       t_generated_artifacts_are_gated_on_their_generators,
